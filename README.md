...
- Sửa, xóa, đổi trạng thái bất kỳ công việc nào.
...
**Chung**
- Tìm kiếm theo tiêu đề/mô tả (debounce 300ms), phân trang, sắp xếp linh hoạt.
...
| Method | Endpoint | Quyền | Mô tả |
|---|---|---|---|
| POST | `/users` | Leader | Tạo tài khoản nhân viên mới |
...
Query params cho `GET /todos`: `keyword`, `status` (`PENDING`\|`IN_PROGRESS`\|`COMPLETED`), `priority` (`LOW`\|`MEDIUM`\|`HIGH`), `assigneeId` (chỉ có tác dụng với Leader), `page` (mặc định 0), `size` (mặc định 10, tối đa 50), `sortBy` (mặc định `createdAt`), `sortDir` (`ASC`\|`DESC`, mặc định `DESC`).
...
│   ├── src/
│   │   ├── api/                          # client.js (axios instance + JWT interceptor), authApi/userApi/todoApi/notificationApi, ws.js
│   │   ├── context/                      # AuthContext (JWT, user, login/logout)
│   │   ├── hooks/                        # useTodos, useEmployees, useNotifications
